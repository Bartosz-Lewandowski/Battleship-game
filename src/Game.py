...
class Game(): 
            ...
            for event in events:
                ...
                if event == ships_laoyout_stage:
                    ...
                    if self.ships_layout_type == 0:
                        ...
                        self.ships_matrix=[[0 for i in range(0, self.board_size -1)] for i in range(0, self.board_size -1)]
                    ...
                    if self.stage == "SET_SHIPS" and self.dragged_ship is not None:
                        ...
                        if tile:
                            for ship in self.draggable_ships:
                                if ship.id == self.dragged_ship.id:
                                    ship.set_new_pos(tile.xpos, tile.ypos)
                                    break
                        self.dragged_ship = None
                        drag = False
                        self.surface.fill((0,0,0))
    ...
    def draw_board(self, chosing_stage=False):
        ...
        if chosing_stage:
            ...
            for ship in self.avalaible_ships:
                self.draggable_ships.append(DraggableShip(self.surface, self.tile_width, self.offset_enemy_grid+50, line_number*1.5*self.tile_width, ship))
                line_number += 1
            self.draw_draggable_ships()
        else:
        ...
        for row_idx, row in enumerate(self.player_board):
            for cell_idx, cell in enumerate(row):
                if row_idx == 0 or cell_idx == 0:
                    ...
                    if self.dragged_ship.draw_rotated:
                        if self.board_size - cell_idx - self.dragged_ship.length + 1 > 0 and self.validate_add_to_matrix(row_idx-1, cell_idx-1, self.dragged_ship.length, self.dragged_ship.id, False): 
                            return cell
                        else:
                            return None
                    else:
                        if self.board_size - row_idx - self.dragged_ship.length + 1 > 0 and self.validate_add_to_matrix(row_idx-1, cell_idx-1, self.dragged_ship.length, self.dragged_ship.id, True):
                            return cell 
                        else:
                            return None
    ...
    def validate_add_to_matrix(self, x, y, length, ship_id, as_row=True):
        is_valid = True
        def check_surrounding(rowcheck, cellcheck):
            surrounding_valid = True
            if rowcheck -1 > 0 and self.ships_matrix[rowcheck -1][cellcheck] != 0 and self.ships_matrix[rowcheck -1][cellcheck] != ship_id:
                surrounding_valid = False
            if rowcheck +1 < self.board_size -1 and self.ships_matrix[rowcheck +1][cellcheck] != 0 and self.ships_matrix[rowcheck +1][cellcheck] != ship_id:
                surrounding_valid = False
            if cellcheck -1 > 0 and self.ships_matrix[rowcheck][cellcheck -1] != 0 and self.ships_matrix[rowcheck][cellcheck -1] != ship_id:
                surrounding_valid = False
            if cellcheck + 1 < self.board_size -1 and self.ships_matrix[rowcheck][cellcheck +1] != 0 and self.ships_matrix[rowcheck][cellcheck +1] != ship_id:
                surrounding_valid = False
            return surrounding_valid
            
        for row_idx, row in enumerate(self.ships_matrix):
            for cell_idx, cell in enumerate(row):
                if as_row and row_idx == y and cell_idx >= x and cell_idx <= x + length -1 and cell != 0 and cell != ship_id:
                    is_valid = False
                if not as_row and cell_idx == x and row_idx >= y and row_idx <= y + length -1 and cell != 0 and cell != ship_id:
                    is_valid = False
        if is_valid:
            for row_idx, row in enumerate(self.ships_matrix):
                for cell_idx, cell in enumerate(row):
                    if self.ships_matrix[row_idx][cell_idx] == ship_id:
                        self.ships_matrix[row_idx][cell_idx] = 0
                    if as_row and row_idx == y and cell_idx >= x and cell_idx <= x + length -1:
                        if  not check_surrounding(row_idx, cell_idx) and False:
                            return False
                        else:
                            self.ships_matrix[row_idx][cell_idx] = ship_id   
                    if not as_row and cell_idx == x and row_idx >= x and row_idx <= x + length -1:
                        if  not check_surrounding(row_idx, cell_idx) and False:
                            return False
                        else:
                            self.ships_matrix[row_idx][cell_idx] = ship_id    
